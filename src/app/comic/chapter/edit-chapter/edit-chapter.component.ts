...
export class EditChapterComponent implements OnInit {
  ...
  private loadChapter() {
    this.chapterService.read(this.chapterId, [this.comicId]).subscribe(
      (response: IResponse<IChapter>) => {
        this.setChapterForm(response.data);
      },
  ...
  public update(): void {
    if (this.updateForm.valid) {
      this.chapterService.update(this.chapterId, this.getChapterForm(), [this.comicId]).subscribe(
        (response: IResponse<IChapter>) => {
          if(!response.message){
            this.toastyService.success(ToastyConfigUtils.getConfig(response.message, MessageConstants.MESSAGE_TITLE_SUCCESS));
          }else{
            this.toastyService.error(ToastyConfigUtils.getConfig(response.message, MessageConstants.MESSAGE_TITLE_ERROR));
          }
        },
        (errorResponse: IErrorResponse) => {
  ...
  private setChapterForm(chapter: IChapter): void {
